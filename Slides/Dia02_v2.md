...
Dada uma função `a -> b`{.haskell} e um `Reader r a`{.haskell}, `fmap`{.haskell} deve criar um `Reader r b`{.haskell}. 
...
Um exemplo desse tipo de Functor é o `Reader a`{.haskell} (chamado de *hom-set*).
